...
def get_args():
    ...
    parser.add_argument('--custom_script',
                        required=True,
                        action='store',
                        help='Name of the customization script')
...
def clone_vm(
        content, template, custom_script, vm_name, si,
    ...
    #Get config of customization script
    custom = si.content.customizationSpecManager.GetCustomizationSpec(name=custom_script)
    ...
    clonespec = vim.vm.CloneSpec(customization=custom.spec)
    ...
    wait_for_task(task)
    print ("VM has been cloned. Name: " + vm_name)
...
def main():
    ...
    if template:
        clone_vm(
            content, template, args.custom_script, args.vm_name, si,
